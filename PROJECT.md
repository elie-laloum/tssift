...
- `fileNames` vide **et** aucune référence ⇒ **sortie 0**, avec `0 errors · 0 files checked`. Ici le zéro est **exact**, et refuser un projet légitimement vide serait notre propre faux négatif.

Ce que la règle 15 interdit n'est pas la sortie `0`, c'est une sortie `0` **fausse**. `projectReferences` est précisément ce qui sépare les deux cas, et il est déjà disponible dans le résultat de `parseJsonConfigFileContent`. L'agent distingue ainsi « ton code est cassé » de « mon invocation est cassée » sans analyser de texte — c'est la thèse du produit appliquée à nos propres modes de panne. Rapport sur stdout, pannes de l'outil sur stderr. `--project` vaut `./tsconfig.json` par défaut, **sans recherche ascendante** : le déterminisme vaut mieux que la magie. Un run propre imprime une ligne plutôt que rien, pour qu'un succès ne ressemble pas à un plantage avalé.
