...
**Ce qui n'est PAS implémenté, dit ici plutôt que passé sous silence.** La règle 2307 ci-dessus — « tout ce qui importe ce module est dérivé », via `ProgramFacts.imports` — reste **non écrite**. **Mais elle n'est plus bloquée, et sa raison d'être différée était fausse.** Le texte de P1 disait « la règle demande un champ que le modèle n'a pas encore (le spécificateur non résolu) » : vérifié le 2026-07-28 sur les trois fixtures 2307 du troisième lot, `ProgramFacts.imports` porte **tous** les spécificateurs tels qu'écrits, résolus ou non — `{"src/callback.ts":["qs"]}` sur `phantom-dependency-pnpm`, dont le module ne résout précisément pas. Le champ était là depuis P0 ; c'est le commentaire de `src/types.ts` qui disait encore « resolved specifiers », corrigé dans le même changement. Le vrai blocage était l'autre moitié de la phrase, l'absence de fixture, et elle est levée : `wrong-tsconfig-paths`, `phantom-dependency-pnpm` et `yarn-pnp-project` portent dix TS2307 sur trois spécificateurs.

Trois choses que ces fixtures disent sur la forme que la règle devra prendre, et qu'aucun raisonnement n'avait données :
...
**Quelle fixture, précisément — la réponse a changé le 2026-07-27.** Cette règle a longtemps été rattachée à `broken-barrel-export`, la quatrième fixture de contrat. C'était une erreur de rangement : **un barrel qui cesse de réexporter un symbole résout parfaitement** — le fichier existe, le module se charge, seul le membre manque — et produit donc du 2305/2724, l'exact opposé d'un 2307 où *rien* ne résout. Les fixtures qui exerceront la règle sont les deux fixtures d'installateur de §9.1 : **dépendance fantôme sous pnpm** et **projet Yarn PnP sans `node_modules`**. La règle 2307 se rattache donc à l'enrichisseur 2307 de P2, avec elles.
