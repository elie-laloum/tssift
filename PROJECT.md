...
interface DiagnosticContext {
  ...
  missing?: string[];  // membres requis d'`expected` absents du type fourni — 2739/2740/2741
}
...
- **`DiagnosticContext.missing`. Ajouté le 2026-08-02, et lu par un seul code.** Les membres **requis** d'`expected` que le type fourni n'a pas. Rempli uniformément par le résolveur de 2739/2740/2741 — il tient déjà le type cible, donc c'est un `getPropertiesOfType` de plus — mais **seul TS2740 en a un lecteur** : c'est le seul des trois qui tronque sa propre liste (à quatre, dès six manquants). Sur 2739 et 2741 le message est déjà complet et la soustraction revient vide, ce qui est le contrôle qui fonctionne et non un cas particulier écrit pour eux. Les membres **optionnels** sont exclus : leur absence n'émet rien, donc les nommer mettrait sous les yeux du lecteur un nom dont aucun diagnostic ne parle.
...
| **2739/2741** | Missing properties | ⭐ | ~~Liste exacte des manquants~~ → **le site de déclaration du type cible** ; TS2739 n'a jamais tronqué sa liste (mesure 2026-08-02, ci-dessous) |
| **2740** | Missing properties, liste tronquée | ⭐ | **Ajouté à cette table le 2026-08-02, sur décision** : le seul des trois où la liste est élidée (à quatre, dès six manquants), donc le seul où « la liste exacte des manquants » est une information neuve. Site de déclaration **+ les membres que le message a comptés sans les nommer** |
| **18047/18048** | Possibly null / undefined | ⭐ | ~~Origine de la nullabilité (où le type devient optionnel)~~ → **la déclaration du symbole nullable, et son type déclaré**. Livré le 2026-08-04 ; l'origine reste non dérivable, la déclaration ne l'a jamais été |
| **18049** | Possibly null **or** undefined | ⭐ | **Ajouté à cette table le 2026-08-04, sur décision.** Même famille, même gabarit, même ancre, même résolveur que 18047/18048 : le laisser dehors ferait sortir en racine isolée le troisième membre d'une cascade dont les deux autres plient, pour une seule et même cause |
| **2551** | Did you mean X | ⭐ | Déjà bon nativement — surtout ne pas dégrader |

Les codes hors table sortent au format natif. **Couvrir 10 codes bien vaut mieux que 60 à moitié** — la table en compte **douze** depuis le 2026-08-04, TS2740 puis TS18049 y ayant été ajoutés délibérément (voir ci-dessous) ; l'esprit de la règle est la couverture entière d'un petit ensemble, pas le nombre lui-même.
...
- **2307 et 18047/18048 manquaient de données, pas de code.** ~~18047 demande une analyse de flot.~~ **Faux, et corrigé le 2026-08-04.** L'analyse de flot est ce que la *charge utile* de cette table demandait — l'origine de la nullabilité — et elle reste hors de portée. Le **lien de causalité** n'en a jamais eu besoin : ce qui est possiblement nul est un symbole **déclaré**, et sa déclaration est le lien structurel ordinaire que §5.1 rule 2 autorisait déjà. Mesuré sur `nullable-chain` : **4/4** sur `proxy` à `settings.ts:13:3`, la ligne que `meta.json` nomme cause racine, et la fixture passe de 4 entrées à 1. Trois documents affirmaient l'impossible — AGENTS.md, CLAUDE.md et `src/codes.ts` — parce que personne n'avait séparé les deux questions. **La limite honnête est ailleurs et elle est structurelle** : une expression sans nom imprimable ne produit pas ces codes du tout, mais TS2531/2532/2533 (`Object is possibly 'null'`), qui ne portent pas de `{0}` — donc aucune expression citée sur laquelle ancrer, et le résolveur tire toute sa correction de cette ancre. Ces trois-là ne pourront jamais entrer dans `CONTEXT_CAPTURE_CODES` ; c'est l'image inversée de la leçon 2305/2724, et une fixture plus deux tests le gravent. **2322 est livré le 2026-08-02, et pas pour la raison que §5.2 donnait** : le chemin de divergence (`a.b[0].c`) reste non dérivable, mais ce qui l'est valait davantage — **où le type cible est déclaré, et, pour une union, ce qu'il autorise réellement**. `Type '"GBP"' is not assignable to type 'Currency'.` ne dit ni où vit `Currency`, ni qu'il vaut `"EUR" | "USD"`. Et il **plie** : `assignability-mismatch`, la fixture écrite pour *interdire* la clé `related`, plie correctement sur le type contextuel — 3/3 sur la ligne 6, celle que `meta.json` nomme cause racine, là où la règle `related` aurait atteint la ligne 9 et deux diagnostics sur trois. **2307 est livré le 2026-08-02**, exactement par où ce diagnostic disait : le canal `ProgramFacts.resolution` (§4), rempli par la source. C'est le septième enrichisseur, et le seul qui ne lit **aucun** `context` — les deux canaux de la règle 4 servent enfin chacun un enrichisseur, ce qui était l'intention du découpage de §3. Il produit au plus trois faits : la ligne `paths` que le spécificateur active, ce que `package.json` déclare (ou ne déclare pas) pour ce paquet, et la topologie d'installation. Deux choses qu'il **ne dit pas**, sur décision : rien de ce qui est réellement posé sur le disque (`qs` est bien présent sous `node_modules/.pnpm/` dans `phantom-dependency-pnpm` — l'atteindre demanderait un parcours de la topologie privée de pnpm ou un parseur YAML), et rien sur ce que chaque installateur fait d'un paquet non déclaré, qui est vrai, documenté, et n'est pas un fait *sur ce projet-ci*. Chiffres : `EVAL.md` § P2/2307.
- **Une inversion mesurée, et les deux moitiés sont vraies.** P2 avait établi que pour un type **objet nommé**, `typeToString` rend le nom et c'est la **liste des propriétés** qui porte l'information. Pour une union de **primitifs**, c'est l'exact inverse : `getPropertiesOfType` répond sur le type *apparent* et rend les cinquante membres de `String` (`charAt`, `blink`, `fontcolor`), tandis que les **constituants** sont toute la charge utile. Aucune des deux ne se généralise, d'où un test sur le type (`hasOwnMembers`) plutôt qu'une règle unique. Vérifié dans les deux sens : `narrowed-union-member` (union d'objets) sort au caractère près comme avant, `assignability-mismatch` gagne `"EUR" | "USD"`.
- **Un enrichisseur ne décrit jamais une déclaration hors des fichiers du programme** — même autorité que §5.1 pour une cause, `ProgramFacts.files`. Étendu à l'enrichissement le 2026-08-02, quand la capture de 2322 a produit le premier cas : `unconstrained-generic` résout son type attendu vers `Map` dans `lib.es2015.collection.d.ts`, et rendait un site que personne n'ouvrira plus la liste des douze méthodes de `Map`. Tout ou rien, parce que les faits d'un diagnostic décrivent le seul symbole résolu : le diagnostic retombe en `confidence: 'low'`, donc au format natif (règle 5).
- **2739/2741 sortent le 2026-08-02, et la mesure a corrigé cette table au passage.** L'entrée ci-dessus demandait « la liste exacte des manquants » au motif que TypeScript tronque la sienne. **Il ne la tronque pas.** Sondé sur 5.9.3 de 1 à 8 propriétés manquantes : 1 ⇒ **TS2741** (qui la nomme), 2 à 5 ⇒ **TS2739** (liste complète, jamais élidée), 6 et au-delà ⇒ **TS2740** (`…, and N more.`, tronquée à quatre). La troncature décrite appartient donc à **TS2740, absent de cette table des dix** — donc hors périmètre tant que personne ne l'y met délibérément. Ce que ces deux codes reçoivent à la place est **le site de déclaration du type cible**, que leurs messages nomment toujours et ne situent jamais.
  **TS2740 a donc été ajouté à cette table, et c'est une décision humaine prise le 2026-08-02** — la règle d'AGENTS.md interdit d'ajouter un code hors table sans la demander. Il reçoit le site de déclaration **et** les membres que son propre message a comptés sans les nommer, calculés par soustraction contre le message verbatim plutôt qu'en prenant la queue de `missing` : la queue n'est la bonne réponse que si TypeScript imprime dans l'ordre de `getPropertiesOfType`, ce qui est une supposition sur ses internes et non une vérification. Sa fixture, `missing-many-properties`, est la seule qui émette ce code.
  Le vrai gain n'est pas là où §5.2 le cherchait : ces deux codes entrent surtout dans **`CONTEXT_CAPTURE_CODES`**, parce que le type cible est une **cause partagée** — N sites de construction d'une interface cassent ensemble le jour où elle gagne un membre requis. Les deux fixtures passent de 3 entrées à 1 (§5.1), et le pliage à cause unique passe de **8/17 à 10/17**. Le coût est de +313 caractères au total, et il est **structurel et non volumétrique** : sous le plafond d'affichage, les trois diagnostics s'impriment encore et l'en-tête s'ajoute par-dessus, exactement comme sur `partial-interface-rename`.
