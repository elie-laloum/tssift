...
interface ProgramFacts {
  ...
  imports: Record<string, string[]>;  // fichier → spécificateurs résolus
...
- **`SourceSpan.file`.** Relatif au dossier du tsconfig résolu, séparateurs POSIX, la racine étant imprimée une fois en tête de rapport. Les chemins hors racine sont normalisés : `<ts-lib>/lib.es5.d.ts` pour les libs TypeScript, suffixe `node_modules/…` depuis la racine de paquet la plus proche. Sans quoi un snapshot embarque `/home/<user>/…` et la version de TS, et meurt en CI comme sur toute autre machine.
