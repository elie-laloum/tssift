...
5. **Le bras modèle B1** (T4) — **livré et mesuré le 2026-07-28.** Harnais `eval/agent/` (`mise exec -- bun run eval:agent`), boucle tool-use sur `fetch` sans dépendance contre un endpoint **OpenAI-compatible** (`/chat/completions`), deux bras A/B, trois outils, `temperature: 0`, n=5, `rootCauseFiles` par `meta.json` comme vérité terrain ; endpoint dans `.env` (gitignoré). **Premier balayage fait — 230 runs, `cx/gpt-5.6-terra`**, et il **tempère H1 plutôt qu'il ne le confirme** : correction 100 % des deux côtés (H2 sans prise), tokens −34 % côté B sur le corpus réel (H1 volumétrique tient), mais **faux départ non réduit** (A 16, B 19 sur 115) — le modèle stube un `.d.ts` sur les erreurs de module, y compris malgré le refus PnP. Détail et tableau dans `EVAL.md` § B1.
