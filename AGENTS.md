...
- **Le mécanisme est lisible, pas mystérieux.** Les fichiers dérivés du bras B sur `hono` sont exactement les quatre qui portent ses sept diagnostics de second ordre, rendus en racines isolées `[2]`–`[8]` sous la cause pliée. Le bras A, lisant 118 diagnostics à plat qui nomment tous `Context`, est allé droit sur `context.ts`. **Hypothèse, non testée : une liste numérotée de racines isolées se lit comme une liste de tâches.**
