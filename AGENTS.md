...
**P0 + B0 livrés le 2026-07-27. P1 est livrée en entier le même jour — T0 → T7.** Le dépôt est initialisé sur `main`, la chaîne d'outils est en place, et `mise exec -- bun run typecheck && … test && … check` passe (**591 tests**, mesuré le 2026-08-04 ; 562 la veille). Depuis le 2026-07-28, le corpus de fixtures est passé de 4 à **22** — la cible de §8 lot B est dépassée (**T0 de B1**). Un lot intermédiaire a dévié du tsconfig canonique pour épuiser les catégories de **configuration** de §7, dont deux fixtures portent une **arborescence installée commitée** (liens `.pnpm`, projet PnP sans `node_modules`) demandant les exceptions `!fixtures/**/…` du `.gitignore` ; le dernier lot (T0) ajoute la dernière catégorie de §7, `missing-type-import`, deux cascades témoins et un second témoin négatif.
...
**Le chiffre à connaître : sur les dix-huit fixtures qui sont des cascades à cause unique, le seuil en plie treize** (cinq avant T1, huit après, puis dix avec 2739/2741, douze avec 2740 et 2322 le 2026-08-02, treize avec 2304/2552 le 2026-08-04). Les cinq autres sortent entre 112 % et 181 %, c'est-à-dire au surcoût de P0. Le pliage tient à **trois** liens structurels : un `declaredAt` identique (neuf fixtures), un spécificateur non résolu partagé (les trois 2307, depuis T1) et un nom manquant partagé (`cannot-find-name`, depuis le 2026-08-04). Les non-pliages sont commités pour rendre le manque **mesurable** : parmi les cinq, seul 18047 (`nullable-chain`) est encore un trou ouvert de §5.2 — 2551 est clos par décision, et 2536 · 2749 · 1484 sont hors table (règle 8). *(Les quatre exclues du dénominateur : `overload-mismatch`, un seul diagnostic, et les trois témoins négatifs à plusieurs racines, `two-independent-roots`, `two-roots-one-file` et `two-missing-names-one-file`.)*
