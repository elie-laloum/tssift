...
Ce qui n'existe pas encore : **18047/18048**, seul code de §5.2 encore bloqué, et par une analyse de flot de contrôle — pas par du code à écrire. **2769 et 2551 sont clos par mesure** (charge utile déjà dans `chain` ; suggestion déjà bonne nativement). **TS2503 et TS2686**, également racines de §5.1, sont **hors périmètre sur décision** — aucune fixture, aucun témoin réel, gabarit différent ; un test le grave pour que ce ne soit pas un oubli qu'on « corrige » en élargissant la liste de codes.
...
**Le chiffre à connaître : sur les dix-neuf fixtures qui sont des cascades à cause unique, le seuil en plie quatorze** (cinq avant T1, huit après, puis dix avec 2739/2741, douze avec 2740 et 2322 le 2026-08-02, treize avec 2304/2552 puis quatorze avec `namespace-import-rename` le 2026-08-04). Les cinq autres sortent entre 112 % et 181 %, c'est-à-dire au surcoût de P0. Le pliage tient à **trois** liens structurels : un `declaredAt` identique (neuf fixtures), un spécificateur non résolu partagé (les trois 2307, depuis T1) et un nom manquant partagé (`cannot-find-name`, depuis le 2026-08-04). `namespace-import-rename` plie sur le premier des trois, son `declaredAt` étant le fichier-module lui-même. Les non-pliages sont commités pour rendre le manque **mesurable** : parmi les cinq, seul 18047 (`nullable-chain`) est encore un trou ouvert de §5.2 — 2551 est clos par décision, et 2536 · 2749 · 1484 sont hors table (règle 8). *(Les quatre exclues du dénominateur : `overload-mismatch`, un seul diagnostic, et les trois témoins négatifs à plusieurs racines, `two-independent-roots`, `two-roots-one-file` et `two-missing-names-one-file`.)*
