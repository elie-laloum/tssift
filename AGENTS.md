...
5. **Le bras modèle B1** (T4) — **harnais livré et validé le 2026-07-28**, `eval/agent/` (`mise exec -- bun run eval:agent`), boucle tool-use sur `fetch` sans dépendance contre un endpoint **OpenAI-compatible** (`/chat/completions`), deux bras A/B, trois outils, `temperature: 0`, n=5, `rootCauseFiles` ajouté à chaque `meta.json` comme vérité terrain du faux départ. Modèle et endpoint pilotés par l'environnement (`OPENAI_BASE_URL`, `OPENAI_API_KEY`, `AGENT_MODEL`). Machinerie validée de bout en bout contre un serveur mock ; **le balayage (230 runs) attend un endpoint configuré** — aucun run modèle réel fait. Détail et protocole dans `EVAL.md` § B1.
