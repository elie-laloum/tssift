...
**P0 + B0 livrés le 2026-07-27. P1 est livrée en entier le même jour — T0 → T7.** Le dépôt est initialisé sur `main`, la chaîne d'outils est en place, et `mise exec -- bun run typecheck && … test && … check` passe (**800 tests**, mesuré le 2026-08-04 ; 772 puis 591 plus tôt le même jour, 562 la veille). Depuis le 2026-07-28, le corpus de fixtures est passé de 4 à **24** — la cible de §8 lot B est dépassée (**T0 de B1**). La vingt-troisième, `namespace-import-rename`, est arrivée le 2026-08-04 par un chemin inhabituel : elle a été écrite **après** le correctif dont elle est le témoin, parce que c'est un dépôt public et non une fixture qui a trouvé le défaut. Un lot intermédiaire a dévié du tsconfig canonique pour épuiser les catégories de **configuration** de §7, dont deux fixtures portent une **arborescence installée commitée** (liens `.pnpm`, projet PnP sans `node_modules`) demandant les exceptions `!fixtures/**/…` du `.gitignore` ; le dernier lot (T0) ajoute la dernière catégorie de §7, `missing-type-import`, deux cascades témoins et un second témoin négatif.

Ce qui existe : les **21** fixtures de contrat · `src/types.ts` + `src/codes.ts` · `TsApiSource` avec **capture sélective de contexte** (`sources/context.ts`, codes 2305 · 2322 · 2339 · 2345 · 2353 · 2554 · 2724 · 2739 · 2740 · 2741) · le canal `ProgramFacts.resolution` (`sources/resolution.ts`, 2026-08-02) · `src/pipeline/` complet — **dedupe, causalité, regroupement, budget**, et `enrich/` à **sept codes** · renderers `json` et `agent-text` · CLI avec sorties 0/1/2 et `--all` / `--budget-tokens` · `eval/measure.ts` et `EVAL.md` · la CI trois axes + garde TS 7 · le harnais modèle `eval/agent/` et le corpus figé de `corpus/` · un **`README.md` public en anglais** et les métadonnées d'empaquetage `0.0.1` (2026-08-01).
...
**P2 est ouvert et sa première tranche est livrée le 2026-08-01** — `src/pipeline/enrich/` n'est plus vide. Six codes ont alors un enrichisseur (**2339, 2353, 2345, 2554, 2305, 2724**) — un septième, **2307**, s'y ajoute le 2026-08-02 — sélectionnés par une règle unique : *un enrichisseur sort quand le fait qu'il produit est déjà capturé et que TypeScript ne l'imprime pas déjà.* Coût mesuré : **+4,0 % de caractères sur le bras B** et **+38 à +83 caractères** seulement sur les cascades de corpus (les rapports B/A publiés ce jour-là sont contaminés par le bogue de `rootLabel` — voir plus bas) — les faits se rendent **une fois par groupe**, pas une fois par diagnostic. Détail et chiffres : `.plans/2026-08-01_p2-enrichment.md` et `EVAL.md` § P2.
...
**2307 est livré le 2026-08-02, et il valide la manière dont P2 avait nommé son blocage.** Le plan disait « il manque un canal `ProgramFacts`, pas du code » ; le canal (`src/sources/resolution.ts`, `ProgramFacts.resolution`) fait 148 lignes, l'enrichisseur 60. C'est le **septième** enrichisseur et le seul qui ne lit **aucun** `context` — les deux canaux de la règle 4 servent enfin chacun un enrichisseur. Il lit des fichiers déclaratifs et rien d'autre (règle 10) : `package.json`, les **noms** des lockfiles, `.pnp.cjs`, la présence d'un `node_modules/`, plus les `paths`/`baseUrl` du compilateur. **Aucun lockfile n'est parsé** — `pnpm-lock.yaml` demanderait un parseur YAML, donc la première dépendance runtime, pour un seul code — et rien ne parcourt `node_modules/.pnpm/`. Coût : **+3,7 %** au total, **0 sur les cinq cascades de corpus** (aucune n'est une cascade de module), et c'est aussi sa limite honnête : sa valeur n'est mesurée sur aucun code réel. Détail : `.plans/2026-08-02_p2-2307.md`, `EVAL.md` § P2/2307.

**Le harnais d'éval mesurait un produit que personne ne livre, et c'est corrigé le 2026-08-02.** `measure.ts` passait au renderer le chemin **absolu** du projet en `rootLabel` là où `run.ts` passe un chemin relatif : le bras B portait `/home/<user>/…`, grossissait de la longueur du chemin de checkout, et **un seul des deux bras était touché**. Conséquence à connaître avant de relire un vieux tableau : **les totaux publiés le 2026-08-01 (16 861 → 17 538, B/A 54 % → 56 %) ne se reproduisent pas** ; le même code donne **16 038 / 51 %** avec le harnais corrigé. Les **deltas** de ce tableau restent justes, la constante s'annulant dans une différence.
...
**2322 est livré le 2026-08-02, et §5.2 est désormais résolu en entier sauf 18047/18048.** Pas pour la raison que la table donnait : le chemin de divergence reste non dérivable, mais **où le type cible est déclaré et ce qu'une union autorise** valait davantage — et **ça plie**. `assignability-mismatch`, la fixture écrite pour *interdire* la clé `related`, plie 3/3 sur le type contextuel, à la ligne 6 que `meta.json` nomme cause racine, là où la règle `related` visait la ligne 9 et n'atteignait que deux diagnostics sur trois. La fixture porte donc maintenant les **deux** moitiés de l'argument : le `related` est la mauvaise clé, le type contextuel en est une bonne.

**Deux corrections générales que 2322 a fait apparaître, et qu'il ne faut pas redécouvrir :**
- **L'inversion objet/primitif.** Pour un type **objet nommé**, `typeToString` rend le nom et c'est la **liste des propriétés** qui informe (constat P2). Pour une union de **primitifs**, exactement l'inverse : `getPropertiesOfType` répond sur le type apparent et rend les **cinquante membres de `String`** (`charAt`, `blink`, `fontcolor`), tandis que les constituants sont toute la charge utile. D'où un test sur le type (`hasOwnMembers`) et non une règle unique — vérifié dans les deux sens, `narrowed-union-member` sort au caractère près comme avant.
- **Un enrichisseur ne décrit jamais une déclaration hors des fichiers du programme.** Même autorité que §5.1 pour une cause : `ProgramFacts.files`. `unconstrained-generic` résout son type attendu vers `Map` dans `lib.es2015.collection.d.ts` et rendait un site que personne n'ouvrira plus les douze méthodes de `Map` — dont `set` et `delete`, qui **faisaient échouer le test de non-prescription de la règle 1**. Tout ou rien, puis repli natif (règle 5).

**La règle 2304/2552 est livrée le 2026-08-04, et c'est une règle de causalité — pas un enrichisseur.** §5.1 listait 2304 en racine quasi certaine depuis le début ; c'était la moitié non écrite de sa liste, et c'est le vrai code de la veille qui l'a débloquée. Cinq faits à ne pas redécouvrir :
- **Elle a demandé d'amender §5.1, délibérément.** §5.1 excluait « même identifiant » et « même nom dans le même fichier » — mais la raison qu'il en donnait est **l'ombrage**, et l'ombrage demande une liaison. `Cannot find name 'X'` est le compilateur affirmant qu'il n'y en a aucune, donc le mécanisme invoqué y est structurellement impossible. L'exception est **étroite** : l'interdit reste entier pour les noms liés, et deux tests nommés sur du 2339 l'y tiennent, intacts.
- **La clé est `(fichier, nom)`, et c'est là que l'analogie avec 2307 s'arrête.** Un spécificateur non relatif désigne le même paquet depuis partout ; un identifiant est scope-local. Même nom dans deux fichiers ⇒ deux entrées.
- **2552 voyage avec 2304**, exactement comme 2724 avec 2305 : le code émis dépend de l'orthographe voisine, pas de la panne. Rien n'est dégradé — tous les membres d'un groupe partagent le nom, donc la même suggestion.
- **Le garde de correction est le `snippet`**, qui joue le rôle que `ProgramFacts.imports` joue pour 2307 : recouper le message contre autre chose que lui-même. Absent ou contredisant ⇒ racine isolée. Les deux gabarits ont été **lus dans `ts.Diagnostics`** de 5.4.5, 5.9.3 et 6.0.3, identiques ; le motif est ancré en début de chaîne, sans quoi il irait chercher la suggestion de 2552 comme clé.
- **La fixture et le vrai code sont en désaccord sur le signe, et les deux sont publiés.** `cannot-find-name` passe de 116 % à **85 %** — seconde fixture sous 100 %, parce que sept sites dépassent le plafond d'affichage. Sur `data-explorer` les entrées passent de 10 à **6** mais les caractères **montent de 596** (68 % → 84 %), aucun groupe n'y ayant plus de deux membres. Le prédicteur est le même depuis P1 : le nombre de sites qu'une cause explique. Détail : `EVAL.md` § P1/2304, `.plans/2026-08-04_p1-2304.md`.

**§5.2 est résolu en entier depuis le 2026-08-04.** 18047/18048 sont livrés, et **18049 est ajouté à la table sur décision humaine** (précédent 2740) — elle compte douze codes. Le blocage annoncé par trois documents n'était pas là où ils le disaient : l'analyse de flot est ce qu'exige la *charge utile* de §5.2 (l'origine de la nullabilité, toujours hors de portée), pas le **lien de causalité**, qui n'est qu'un `declaredAt` ordinaire. La limite réelle est structurelle : **TS2531/2532/2533** — la variante anonyme, sans `{0}` donc sans ancre — ne pourront jamais être capturés, et `private-fields-and-anonymous-nullish` plus deux tests le gravent. Ce qui n'existe pas encore : **2769 et 2551 sont clos par mesure** (charge utile déjà dans `chain` ; suggestion déjà bonne nativement). **TS2503 et TS2686**, également racines de §5.1, sont **hors périmètre sur décision** — aucune fixture, aucun témoin réel, gabarit différent ; un test le grave pour que ce ne soit pas un oubli qu'on « corrige » en élargissant la liste de codes.

**B3 (2026-08-04) est un résultat négatif pour H1, et c'est la campagne la plus complète du dépôt — lire `EVAL.md` § B3 avant de citer quoi que ce soit d'autre.** Première campagne où les trois préconditions de B2 sont remplies : métrique réparée, corpus élargi sur du code public, deux bras dans la même campagne, échantillonnage à `temperature: 1`. Sur **6 cibles complètes, 30 runs par bras** : le bras B coûte **106 %** des tokens de A, corrige **28/30 contre 30/30**, et fait **9 faux départs contre 5**. Sur aucune métrique il n'est devant. Trois faits à ne pas redécouvrir :
- **H1 tient là où la cascade est propre et s'inverse là où elle a des débris.** Quatre cibles sur six sont à 34–77 % des tokens de A sans coût de comportement — ce sont les entrées anonymisées, une cause et rien d'autre. `hono`, la première cible du dépôt à porter de vrais diagnostics de **second ordre**, est à **141 %** des tokens, **8,2 tours contre 5,8**, et **80 % de faux départ contre 0 %**.
- **Le mécanisme est lisible, et l'explication évidente est fausse — testée le jour même.** Les fichiers dérivés du bras B sur `hono` sont exactement les quatre qui portent ses sept diagnostics de second ordre, rendus en racines isolées `[2]`–`[8]` sous la cause pliée. L'hypothèse « une liste numérotée se lit comme une liste de tâches » a été implémentée (section non numérotée, en-tête `N diagnostics with no cause established:`) et rejouée sur `hono` : **le faux départ reste à 80 %, les mêmes quatre fichiers**, et les tokens empirent. **Réfutée, changement annulé, §6 refermé.** Ce n'est pas la forme de la liste : le modèle édite ces fichiers parce qu'il les **voit**, et c'est le pliage qui les rend visibles — 111 diagnostics sur 118 se réduisent à une ligne, donc sept qui pesaient 6 % du texte du bras A en pèsent la moitié côté B. **Le pliage augmente la saillance relative de ce qui ne plie pas.** Problème ouvert, pas bogue à corriger ; aucun en-tête ne le règle, et déclarer « ce diagnostic disparaîtra quand l'autre sera corrigé » n'est pas dérivable du capturé (§5.1) ni permis (règle 1). Détail : `EVAL.md` § « The to-do-list hypothesis ».
- **Les caractères de B0 ne prédisent pas les tokens du bras modèle.** `hono` sort à 10 % de `tsc` en caractères et à 141 % en tokens : dans une boucle d'agent, le coût est dans les tours, pas dans le premier message. Un rapport plus court qui fait faire deux tours de plus coûte plus cher.
...
- **La métrique de faux départ était fausse sur `order-book`, et elle est réparée le 2026-08-04.** Son `expectedFix` donnait deux correctifs comme également valides puis appelait le premier un faux départ, et `rootCauseFiles` ne contient que la déclaration : mettre à jour les lectures — correctif que la fixture déclare valide — était scoré 100 %. L'audit a nommé la seconde cible que B2 laissait anonyme : **`shape-tag-renamed`**, même patron de phrase (« or equivalently … both settle »). Les trois autres ne déclarent qu'un correctif et sont intactes. **La réparation ne passe par aucune des deux voies que B2 proposait** — élargir `rootCauseFiles` aurait autorisé 17 fichiers sur 23 et cessé de mesurer la cascade de renommage ; restreindre `expectedFix` aurait décrété non valide un correctif qui compile. Les catégories sont donc **séparées** : `meta.json` peut déclarer **`consumerFiles`**, la route large que sa propre prose admet, scorée dans **sa propre colonne** ; le faux départ redevient ce que dit §7, une écriture dans un fichier qu'aucun correctif n'implique (5 fichiers sur 23 ici). `test/ground-truth.test.ts` grave l'existence des chemins, la disjonction des deux ensembles et le patron de phrase fautif. **Les taux publiés de B1 et B2 restent ceux de l'ancien scorage** et ne sont pas recalculables sans rejouer.

**Un enrichisseur P2 s'est déclenché pour la première fois sur du vrai code le 2026-08-03**, sur un monorepo Nx privé (`keyzia/data-explorer`, TS 5.8.3, 919 fichiers) — `EVAL.md` § « Real code ». Sondage ponctuel, pas une campagne : un projet, un run, pas de bras modèle. Trois faits à ne pas redécouvrir :
- **Le pliage et l'enrichissement marchent sur du code que personne n'a écrit pour nous.** Dix TS2339 sur quatre fichiers plient en une entrée (90 %) sur `class 'NotificationHelpers'`, dont l'enrichisseur dit `has 1 property: prototype` — la classe est réellement vide, vérifié. Sortie à **68 %** de `tsc` brut, `--all` à 137 %.
- **2304/2552 ne plient pas, et c'est là un vrai manque.** Six TS2304 et deux TS2552 pour **quatre noms distincts** sortent en huit entrées ; les six 2304 sont dans un seul fichier. La clé est le **nom**, comme 2307 avant T1. **La fixture `cannot-find-name` ne pouvait pas le montrer** : elle porte un nom sur un site, donc son pliage 0 % est une propriété de la fixture et non du code. C'est le premier témoin réel d'une cascade 2304 avec répétition.
- **La limite honnête de 2307 en monorepo.** Sur `libs/chat-components`, dont le dossier n'a aucun `package.json`, le seul fait produit est `no node_modules directory at the project root` — vrai, correctement borné (`resolution.ts` teste `join(root, "node_modules")` et ne remonte pas), et tout ce que la décision 3 autorise à dire. Comportement conforme à la spec ; c'est la branche la plus prudente qui est aussi la moins informative. Observation, pas bogue.
...
| `nullable-chain` | 18047 | 4 → 1 | ~~cascade réelle que le seuil refuse~~ — **plie depuis le 2026-08-04** : le flot de contrôle bloquait la charge utile, jamais le lien |
| `missing-required-property` | 2741 | 3 → 1 | un `related` qui **désigne bien** la cause, et reste inutilisé — le pliage se fait sur l'interface (7:1), le `related` pointe la propriété (10:3) |
| `missing-multiple-properties` | 2739 | 3 → 1 | le jumeau **multi-membres** de 2741 ; le témoin qui manquait à 2739 |
| `assignability-mismatch` | 2322 | 3 → 1 | un `related` qui **désigne la mauvaise ligne** — la réfutation de la règle « related », et depuis le 2026-08-02 la preuve que le type contextuel est une bonne clé : 3/3 sur la ligne 6 |
...
| `cannot-find-name` | 2304 | 7 → 1 | la **seconde moitié** de la liste de racines de §5.1 ; seule fixture où un diagnostic **court** dépasse le plafond par le nombre — d'où 85 %, la seconde sous 100 % |
| `two-missing-names-one-file` | 2304 · 2552 | 5 → **2** | le témoin négatif de la clé `(fichier, nom)` ; le seul TS2552 commité, et la ligne 23 qui porte un diagnostic de chaque groupe |
...
| `namespace-import-rename` | 2339 | 4 → 1 | cause = un module atteint par **import de namespace** ; la seule dont les messages bruts portent un chemin **absolu**, et le témoin du correctif de normalisation du 2026-08-04 |
| `private-fields-and-anonymous-nullish` | 2339 · 2531 | 2 → 1 (+1 racine) | deux témoins d'un coup : une classe **majoritairement `#privée`** (filtre du 2026-08-04) et la variante **anonyme** de la famille 18047, qui ne pliera jamais |

**Le chiffre à connaître : sur les vingt fixtures qui sont des cascades à cause unique, le seuil en plie seize** (cinq avant T1, huit après, puis dix avec 2739/2741, douze avec 2740 et 2322 le 2026-08-02, treize avec 2304/2552, quatorze avec `namespace-import-rename`, puis seize avec la famille 18047 et `private-fields-and-anonymous-nullish` le 2026-08-04). Les quatre autres sortent entre 112 % et 181 %, c'est-à-dire au surcoût de P0. Le pliage tient à **trois** liens structurels : un `declaredAt` identique (neuf fixtures), un spécificateur non résolu partagé (les trois 2307, depuis T1) et un nom manquant partagé (`cannot-find-name`, depuis le 2026-08-04). `namespace-import-rename` plie sur le premier des trois, son `declaredAt` étant le fichier-module lui-même. Les non-pliages sont commités pour rendre le manque **mesurable** : **il ne reste aucun trou ouvert de §5.2** depuis que 18047 plie — 2551 est clos par décision, et 2536 · 2749 · 1484 sont hors table (règle 8). *(Les quatre exclues du dénominateur : `overload-mismatch`, un seul diagnostic, et les trois témoins négatifs à plusieurs racines, `two-independent-roots`, `two-roots-one-file` et `two-missing-names-one-file`.)*
...
4. ~~**Un corpus réel figé plus large** (T3)~~ — **livré le 2026-08-04.** Trois entrées publiques s'ajoutent : `hono` (propriété de classe, TS2339, B/A **10 %**), `zod` (symbole de module, TS2339, 99 → **1 entrée**, B/A **5 %**) et `date-fns` (arité, TS2554, B/A **25 %**) — trois dépôts MIT sans dépendance runtime, trois familles de liens structurels distinctes. **Rien n'est vendoré** : `eval/corpus.json` porte un `remote` + un sha, `build-corpus.mjs` miroite une fois dans `.cache/` (sans copie de travail) et `git archive` en sort un arbre figé ; `--offline` saute ce qui n'est pas déjà miroité. Chaque entrée écrit son `tsconfig.json`, les trois vrais fichiers étant soit des tsconfig solution, soit des configs qui étendent un paquet de dev non installé. **Ce sont les meilleurs rapports H1 du dépôt, et les premiers sur du code écrit par des gens qui ne nous connaissent pas.** Deux défauts ont été trouvés dans l'heure par ce seul élargissement — le chemin absolu des causes-module (corrigé) et les champs privés `#x` dans les listes de propriétés (consigné, décision ouverte). Détail : `EVAL.md` § T3.
...
**1. L'enrichisseur `2307` — c'est là que ça se joue, et il est livré depuis le 2026-08-02.** Il distingue « paquet absent de `package.json` » de « types manquants » de « chemin cassé » de « dépendance non déclarée mais résolue par hoisting ». Ces distinctions dépendent entièrement de la topologie installée, qui n'est pas la même selon l'installateur :
...
En pratique — **tout ce qui suit est en place**, dans `src/sources/resolution.ts` pour la lecture et `src/pipeline/enrich/2307.ts` pour la formulation :
- Détecter l'installateur par lockfile (`package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `bun.lock`) et l'exposer comme `Fact` de type `origin`. **Deux lockfiles ⇒ `installer: unknown` et la liste**, jamais un gagnant par priorité : ce serait une supposition imprimée comme un fait.
...
- Le code, les identifiants et les commentaires sont en **anglais**. La documentation projet (`PROJECT.md`, `AGENTS.md`, les plans de `.plans/`) est en **français**. **Exception depuis le 2026-08-03 : `EVAL.md` est en anglais**, comme le `README.md`. Raison : ses chiffres sont écrits pour être reproduits et cités par quelqu'un qui n'est pas nous, et la sortie du renderer qu'il cite partout est anglaise (règle 13) — c'était le seul document où le lecteur changeait de langue au milieu d'un tableau. Ne pas le retraduire.
...
```
fixtures/<nom-kebab>/
  meta.json     { rootCause, expectedFix, tags, difficulty, purpose?, deviatesFromCanonicalConfig?, rootCauseFiles?, consumerFiles? }
  before/       projet cassé, ne compile pas
```

**Il n'y a pas de `after/`, et c'est l'état réel des vingt-et-une fixtures** — la description ci-dessus en annonçait un jusqu'au 2026-08-02, sans qu'aucune ne l'ait jamais porté. L'état corrigé attendu vit dans `meta.json.expectedFix`, en prose, qui est ce que lit le harnais d'éval ; un `after/` dupliquerait un projet entier pour une information que le harnais n'ouvre pas. `rootCauseFiles` est ce que le bras modèle compare pour compter les faux départs — il peut nommer un fichier **absent** de `before/` quand le correctif consiste à le créer (`cannot-find-name` et `two-missing-names-one-file` nomment tous deux `src/money.ts`, le module supprimé qu'il faut réintroduire), et `write_file` crée aussi bien qu'il écrase.

`consumerFiles` est **optionnel** et n'a de sens que sur une cascade où deux correctifs compilent : la déclaration d'un côté, tous ses consommateurs de l'autre. Il ne se remplit **que** si `expectedFix` concède explicitement la seconde route — sinon il n'y a pas deux routes, il y a un correctif et des faux départs. Une écriture dans `consumerFiles` n'est **pas** un faux départ ; elle est comptée dans sa propre colonne (`consumer route`). Deux entrées de corpus le portent, `order-book-field-renamed` et `shape-tag-renamed`, et les deux ensembles sont **disjoints** par construction — un test le vérifie. Colonne `—` et colonne `0 %` disent deux choses différentes : la fixture n'offre pas de route large, contre elle en offre une que le modèle a déclinée.
